...
class Module_type {
    ...
    function install_dependency($dependency_data) {
        ...
        if ($spark->verify(false)) { // if not installed, install
            $spark->retrieve();
            $spark->install();
            Module_utils::notice("Installed dependency: $spark->name to " . $spark->installed_path());
        }
        else {
            Module_utils::warning("Dependency $spark->name is already installed.");
        }
    }
    ...
    function verify($break_on_already_installed = true)
    {
        ...
        if ($this->data->is_deactivated)
        {
            ...
            throw new Module_exception($msg);
        }
        // see if this is unsupported
        if ($this->data->is_unsupported)
        {
            Module_utils::warning('This spark is no longer supported.');
            Module_utils::warning('You can keep using it, or look for an alternate');
        }

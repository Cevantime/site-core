...
class ApiMediaFileUploadTest extends BaseTest {
	...
	public function testMediaFile() {
		...
		$payload = null;
		$params = null;
		$this->assertEquals(false, Google_MediaFileUpload::process(null, $payload, $params));
	...
	public function testGetUploadType() {
		...
		$this->assertEquals('resumable', Google_MediaFileUpload::getUploadType(null, $payload, $params));

		// Test custom uploadType values.
		$params = array('uploadType' => array('value' => 'foo'));
		$this->assertEquals('foo', Google_MediaFileUpload::getUploadType(null, $payload, $params));

		// No data available for the upload.
		$params = array();
		$this->assertEquals(false, Google_MediaFileUpload::getUploadType(false, $payload, $params));

		// Test file uploads
		$params = array('file' => array('value' => '@/foo'));
		$this->assertEquals('media', Google_MediaFileUpload::getUploadType(null, $payload, $params));
		...
		$params = array('data' => array('value' => 'foo'));
		$this->assertEquals('multipart', Google_MediaFileUpload::getUploadType(array('a' => 'b'), $payload, $params));
	}

	public function testProcessFile() {
		$this->assertEquals(array('postBody' => array('file' => '@/tmp')), Google_MediaFileUpload::processFileUpload('@/tmp', false));

		$this->assertEquals(array('postBody' => array('file' => '@/tmp')), Google_MediaFileUpload::processFileUpload('/tmp', false));

		$this->assertEquals(array('postBody' => array('file' => '@../tmp')), Google_MediaFileUpload::processFileUpload('../tmp', false));

		$this->assertEquals(array(), Google_MediaFileUpload::processFileUpload('', false));
	}
